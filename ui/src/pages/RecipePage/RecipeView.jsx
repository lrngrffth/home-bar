      ...
      <div className="flex flex-col w-full h-screen pb-8">
        ...
        <div className="flex h-full pt-12 w-full px-12 gap-3">
          <div className="flex flex-col gap-3">
            <div className="flex flex-col h-5/6 bg-dusk-rose w-52 rounded-3xl text-xl py-3 px-3 gap-1 overflow- no-scrollbar">
              <div className="font-bold px-3">Ticket</div>
              <div className="h-full px-3">
                {recipePageStore.drinks.map((drink, key) => (
                  <div 
                    className={clsx("pl-6 hover:bg-cotton-candy hover:bg-opacity-20 font-light cursor-pointer", {"bg-cotton-candy": recipePageStore.currentDrink == key, "bg-opacity-50":  recipePageStore.currentDrink == key})} 
              ...
              <Button dark={"true"} onClick={() =>navigate("/")} className="cursor-pointer">Close ticket</Button>
            </div>
            <Button onClick={() =>navigate("/order")} className=" cursor-pointer">Back</Button>
          </div>
          <div className="flex flex-col w-full pl-8 font-inter font-light text-5xl pr-48 overflow-scroll no-scrollbar mb-24">
              ...
              {recipePageStore.recipes[recipePageStore.currentDrink]["Ingredients"]["Add Ons"] && 
                <>
                  ...
                  {recipePageStore.recipes[recipePageStore.currentDrink]["Ingredients"]["Add Ons"].map((ingredient, key2) => (
                    <li className="ml-16 pb-2" key={key2}>{ingredient}</li>
                  ))}
                </>
              }
            </ul>
                  ...
                  {item != "Combine" &&
                    <div className="pb-4">
                      ...
                      {recipePageStore.recipes[recipePageStore.currentDrink]["Directions"][item].map((step, key2) => (
                        <li className="ml-16 pb-2" key={key2}>{step}</li>
                      ))}
                    </div>
                  }
                </>
