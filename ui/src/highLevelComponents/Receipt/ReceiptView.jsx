...
const Receipt = observer(() => {
  ...
  return (
    <div className="flex flex-col text-lg items-center w-96 h-screen font-abhaya justify-between py-6 overflow-scroll no-scrollbar">
        ...
        <div className="flex flex-col h-full w-full">
          ...
          <div className="flex flex-col gap-4">
            {receiptStore.items.map((drink, i) => (
              <div className="flex flex-col w-full pr-4 gap-1" key={i}>
                <div className="flex w-full items-center gap-2">{i == receiptStore.selectedDrink ? <ChevronRight className="h-4 w-auto"/> : <div className="pl-4"></div>}<div onClick={() => receiptStore.selectDrink(i)} className="w-full cursor-pointer">Drink #{i + 1}</div>{receiptStore.items.length > 1 && <DashCircle className="items-end h-4 w-auto cursor-pointer" onClick={() => receiptStore.removeDrink(i)}/>}</div>
                {receiptStore.items[i].map((item, key) => (
                  <div className="flex w-full pl-8" key={key}>
                    <div className="flex flex-col w-full">
                      <div className="flex w-full items-start justify-start gap-2">
                        {i == receiptStore.selectedDrink 
                          ? <DashCircle className="w-4 mt-1 cursor-pointer" onClick={() => receiptStore.removeItem(i, key)}/> 
                          : <></>
                        }
      ...
      <div className="flex flex-col px-4 w-full ">
        <div className="flex w-full items-center justify-center gap-2 text-center mb-3 cursor-pointer" onClick={() => receiptStore.addDrink()}><PlusCircle/>Add Drink</div>
        <div className="w-full py-2 bg-paw-print bg-contain bg-repeat-x h-2 my-2 "/>
        ...
        <div className="flex w-full mx-auto justify-center cursor-pointer"><Button onClick={() => {receiptStore.makeOrder(); navigate('/recipe');}}>Order Up!</Button></div>
      </div>
    </div>
  );
})
