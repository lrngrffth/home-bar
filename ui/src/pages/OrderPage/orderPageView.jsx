...
import loadingGif from "../../images/cats_playing.gif";
...
const MainPage = observer(() => {
  ...
  return (
    <div className="flex items-center justify-center w-full h-screen max-h-screen overflow-hidden">
      {orderPageStore.loading ? 
        <div className="bg-cotton-candy flex flex-col w-full h-screen items-center justify-center gap-6">
          <div className="mt-24">
            <img src={loadingGif}/>
          </div>
          <div className="text-2xl text-dusk-rose mb-24">Loading . . .</div>
        </div> :
      <>
        <div className="flex flex-col w-full h-screen bg-deep-marroon overflow-scroll no-scrollbar pb-8">
          <div className="relative text-dusk-rose font-semibold font-abhaya text-6xl mx-6 mt-4">
            ...
            <div className="flex gap-3 text-dusk-rose font-abhaya text-xl mx-auto mt-3 divide-x absolute right-0 top-0 font-medium">
              <div className="w-fit pl-3 border-dusk-rose cursor-pointer text-nowrap cursor-pointer" onClick={() => navigate('/')}>Home</div>
          ...
          <div className="flex gap-3 text-dusk-rose font-abhaya text-xl mx-auto mt-3 divide-x">
            {orderPageStore.itemTypes.map((item, key) => (
              <div key={key} className={clsx("w-fit pl-3 border-dusk-rose cursor-pointer text-nowrap cursor-pointer", {'font-semibold': item == orderPageStore.itemType, 'underline': item == orderPageStore.itemType})} onClick={() => orderPageStore.selectItemType(item)}>{item}</div>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-0 mt-12 mx-12">
            <div className="flex mr-auto text-dusk-rose">
              {Object.keys(orderPageStore.subTypes[orderPageStore.itemType]).map((item, key) => (
                <div 
                  className={clsx("flex border border-dusk-rose rounded-t-lg w-fit min-w-24 justify-center py-1 text-nowrap px-4 cursor-pointer", {"bg-dusk-rose": item == orderPageStore.subType[orderPageStore.itemType], "text-deep-marroon": item == orderPageStore.subType[orderPageStore.itemType]})}
            ...
            <div className="flex flex-col items-center justify-center border border-dusk-rose text-cotton-candy divide-y divide-dusk-rose overflow-scroll no-scrollbar">
                ...
                Object.keys(orderPageStore.subTypes[orderPageStore.itemType][orderPageStore.subType[orderPageStore.itemType]]["items"]).map((type, key) => (
                  <>
                    <div className="relative flex h-12 bg-dusk-rose text-deep-marroon w-full items-center justify-center text-2xl font-abhaya font-bold">
                      {type}
                      <>{type in orderPageStore.subTypes[orderPageStore.itemType][orderPageStore.subType[orderPageStore.itemType]]["background"] && <InfoCircle className="absolute right-4 cursor-pointer" onClick={() => orderPageStore.openInfoModal(type)}/>}</>
                    </div>
                    <Modal
                    ...
                    >
                      <div className="relative flex flex-col bg-cotton-candy py-3 px-6 text-dusk-rose gap-3 max-w-xl">
                        ...
                        <XCircle className="absolute right-10 top-10 text-xl text-deep-marroon cursor-pointer" onClick={() => orderPageStore.closeInfoModal()}/>
                        {orderPageStore.infoModalOpen && Object.keys(orderPageStore.subTypes[orderPageStore.itemType][orderPageStore.subType[orderPageStore.itemType]]["background"][orderPageStore.infoModalOpen]).map((quality, key) => (
                          <div className="flex flex-col px-3" key={key}>
                            ...
                            {Array.isArray(orderPageStore.subTypes[orderPageStore.itemType][orderPageStore.subType[orderPageStore.itemType]]["background"][orderPageStore.infoModalOpen][quality]) ? 
                              orderPageStore.subTypes[orderPageStore.itemType][orderPageStore.subType[orderPageStore.itemType]]["background"][orderPageStore.infoModalOpen][quality].map((item, key) => (
                                <div className="flex items-center pl-2" key={key}><Dot/>{item}</div>
                              )) :
                    ...
                    {orderPageStore.subTypes[orderPageStore.itemType][orderPageStore.subType[orderPageStore.itemType]]["items"][type].map((item, key) => (
                      <div className={clsx("flex flex-col w-full gap-1 py-3 relative active:bg-cotton-candy active:opacity-60 cursor-pointer", {"text-dusk-rose": item["properties"]["InStock"]["select"] ? item["properties"]["InStock"]["select"]["name"] == "no" : true})} onClick={() => {orderPageStore.needsSecondStep(item) ? orderPageStore.openOrderModal(item) : receiptStore.addItem(item, null, null, orderPageStore.itemType)}} key={key}>
                        ...
                        <div className="flex w-full gap-2 items-end justify-end pr-4">{item["properties"]["Temp"]["multi_select"].map((temp, i) => (<Temp temperature={temp["name"]} color="pink" key={i}/>))}</div>
                      </div>
                    ))}
                  </>
                ))
              ...
              (orderPageStore.subTypes[orderPageStore.itemType][orderPageStore.subType[orderPageStore.itemType]]["items"]).map((item, key) => (
                  <div className={clsx("flex flex-col w-full gap-1 py-3 relative active:bg-cotton-candy active:opacity-60 cursor-pointer", {"text-dusk-rose": item["properties"]["InStock"]["select"]["name"] == "no"})} onClick={() => {orderPageStore.needsSecondStep(item) ? orderPageStore.openOrderModal(item) : receiptStore.addItem(item, null, null, orderPageStore.itemType)}} key={key}>
                    <div className="absolute right-4">{item["price"]}</div>
              ...
              >
                {orderPageStore.orderModalOpen ?
                  <div className="relative flex flex-col bg-cotton-candy py-3 px-6 text-dusk-rose gap-3 max-w-xl">
                    ...
                    <XCircle className="absolute right-10 top-10 text-xl text-deep-marroon cursor-pointer" onClick={() => orderPageStore.closeOrderModal()}/>
                    <div className="flex flex-col px-3 w-full items-center justify-center pb-3 gap-6">
                      ...
                      <div className="w-full flex justify-end pr-6"><Button className="w-24 cursor-pointer" onClick={() => orderPageStore.submitOrderFromModal()}>Add Drink</Button></div>
                    </div>
                    <div className="w-full py-2 bg-paw-print bg-contain bg-repeat-x h-2 "/>
                  </div>
                : <div></div>}
